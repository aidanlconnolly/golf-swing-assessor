...
- **Pose overlay (Phase 2)** — `setupPose()` creates one `PoseLandmarker` (VIDEO mode) per lane sharing a single `FilesetResolver`. `attachOverlay()` drives detection off `video.requestVideoFrameCallback` during playback, plus `seeked`/`loadeddata` listeners so the skeleton updates while frame-stepping/scrubbing paused. `drawPose()` maps the 33 normalized BlazePose landmarks onto the letterboxed (object-fit: contain) video rect and draws connectors + dots; `KEY_JOINTS` (shoulders/elbows/wrists/hips/knees) are highlighted. GPU delegate with CPU fallback. Detection timestamps are forced monotonic per lane.
